...
import {
  ...
  CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, CircleHelp, Code2, Database, Download, ExternalLink,
...
import { opportunities, outreachStatusOptions, phdTargets as defaultPhdTargets, statusOptions } from './data'
...
const blankAccountData: AccountData = { profile: defaultProfile, applications: [], outreachRecords: [], phdTargets: defaultPhdTargets }
...
type PhdTargetForm = Omit<PhdTarget, 'id' | 'directions'> & { directions: string }
type ApplicationSort = 'date-desc' | 'priority-desc' | 'deadline-asc' | 'company-asc'
const initialTargetForm: PhdTargetForm = { group: '高校', institution: '', city: '', unit: '', directions: '', priority: '重点', admissionsUrl: '', facultyUrl: '', nextAction: '', note: '', suggestedMentor: '' }
const priorityWeight: Record<Application['priority'], number> = { 高: 3, 中: 2, 低: 1 }
...
function mergeUniqueById<T extends { id: string }>(current: T[], incoming: T[]) {
  ...
  return [...current, ...incoming.filter(item => item?.id && !existingIds.has(item.id))]
}
function fillBlankProfile(current: CandidateProfile, incoming: Partial<CandidateProfile>) {
  return Object.fromEntries(Object.entries(current).map(([key, value]) => [key, value || incoming[key as keyof CandidateProfile] || ''])) as unknown as CandidateProfile
}
...
function App({ account, onSessionEnd }: { account: LocalAccount, onSessionEnd: () => void }) {
  ...
  const [phdTargets, setPhdTargets] = useState<PhdTarget[]>(initialAccountData.phdTargets)
  ...
  const [applicationSort, setApplicationSort] = useState<ApplicationSort>('date-desc')
  const [applicationPageSize, setApplicationPageSize] = useState<10 | 20>(10)
  ...
  const [targetForm, setTargetForm] = useState<PhdTargetForm>(initialTargetForm)
  ...
  useEffect(() => {
    saveAccountData(account.id, { profile, applications, outreachRecords, phdTargets })
  }, [account.id, applications, outreachRecords, phdTargets, profile])
  useEffect(() => {
  ...
  const visibleOpportunities = useMemo(() => matchedOpportunities.filter(o => {
    ...
    if (oppFilter === '央国企') return o.tags.includes('央企')
    return true
  }).sort((a, b) => b.match.fit - a.match.fit), [matchedOpportunities, oppFilter])

  const visibleTargets = useMemo(() => phdTargets.filter(target => {
    const groupMatch = targetFilter === '全部目标'
      || (targetFilter === '中科院' && target.group === '中科院/国科大')
      || (targetFilter === '高校' && target.group === '高校')
    const query = search.trim().toLowerCase()
    return groupMatch && (!query || `${target.institution}${target.unit}${target.directions.join('')}`.toLowerCase().includes(query))
  }).map(target => ({ ...target, match: targetMatch(target, profile) })).sort((a, b) => b.match.fit - a.match.fit), [phdTargets, profile, search, targetFilter])
  ...
  const filteredApps = useMemo(() => applications.filter(app => {
    ...
    if (applicationSort === 'priority-desc') return priorityWeight[b.priority] - priorityWeight[a.priority] || (Date.parse(b.appliedAt || '1970-01-01') - Date.parse(a.appliedAt || '1970-01-01'))
    if (applicationSort === 'deadline-asc') return (Date.parse(a.deadline || '2999-12-31') - Date.parse(b.deadline || '2999-12-31'))
    if (applicationSort === 'company-asc') return a.company.localeCompare(b.company, 'zh-CN')
    return (b.appliedAt ? Date.parse(b.appliedAt) : 0) - (a.appliedAt ? Date.parse(a.appliedAt) : 0)
  }), [applicationSort, applications, search, statusFilter])
  const applicationPageCount = Math.max(1, Math.ceil(filteredApps.length / applicationPageSize))
  const safeApplicationPage = Math.min(applicationPage, applicationPageCount)
  const paginatedApps = useMemo(() => filteredApps.slice((safeApplicationPage - 1) * applicationPageSize, safeApplicationPage * applicationPageSize), [applicationPageSize, filteredApps, safeApplicationPage])

  useEffect(() => { setApplicationPage(1) }, [applicationPageSize, applicationSort, search, statusFilter])
  useEffect(() => { setApplicationPage(page => Math.min(page, applicationPageCount)) }, [applicationPageCount])
  ...
  const openTarget = (target?: PhdTarget) => { setEditingTargetId(target?.id || null); setTargetForm(target ? { ...target, directions: target.directions.join('、') } : initialTargetForm); setTargetOpen(true) }
  const saveTarget = (event: React.FormEvent) => {
    event.preventDefault()
    if (!targetForm.institution.trim()) return
    const directions = targetForm.directions.split(/[、,，;/；|\n]+/).map(item => item.trim()).filter(Boolean)
    const target: PhdTarget = { ...targetForm, institution: targetForm.institution.trim(), unit: targetForm.unit.trim(), directions, id: editingTargetId || crypto.randomUUID() }
    setPhdTargets(list => editingTargetId ? list.map(item => item.id === editingTargetId ? target : item) : [target, ...list])
    setTargetOpen(false)
    setToast(editingTargetId ? '目标院校已更新' : '目标院校已添加')
  }
  const removeTarget = (id: string) => {
    if (!window.confirm('确认移除这个目标院校吗？已有导师套磁记录会继续保留。')) return
    setPhdTargets(list => list.filter(item => item.id !== id))
    setToast('目标院校已移除，套磁记录未受影响')
  }
  const exportData = () => { const blob = new Blob([JSON.stringify({ profile, applications, outreachRecords, phdTargets, exportedAt: new Date().toISOString() }, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `OfferFlow-数据备份-${new Date().toISOString().slice(0, 10)}.json`; anchor.click(); URL.revokeObjectURL(url); setToast('备份文件已导出') }
  const importData = (event: React.ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { try { const data = JSON.parse(String(reader.result)); if (!Array.isArray(data.applications)) throw new Error(); setApplications(current => mergeUniqueById(current, data.applications)); if (Array.isArray(data.outreachRecords)) setOutreachRecords(current => mergeUniqueById(current, data.outreachRecords)); if (Array.isArray(data.phdTargets)) setPhdTargets(current => mergeUniqueById(current, data.phdTargets)); if (data.profile) setProfile(current => fillBlankProfile(current, data.profile)); setToast('备份已安全合并，现有数据未覆盖') } catch { setToast('文件格式不正确') } }; reader.readAsText(file); event.target.value = '' }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="content page-content">
        ...
        {activeNav === '博士申请' && <>
          <PageTitle title="博士申请与套磁" description="目标院校可增删编辑，并按你的研究方向实时排序。" icon={<GraduationCap/>}/>
          <section className="phd-section"><div className="outreach-metrics"><MiniMetric label="目标单位" value={phdTargets.length} hint={`${phdTargets.filter(target => target.group === '中科院/国科大').length} 个中科院体系`}/><MiniMetric label="已建记录" value={outreachStats.total} hint="按导师管理"/><MiniMetric label="已联系" value={outreachStats.contacted} hint="邮件已发出"/><MiniMetric label="有效回复" value={outreachStats.replied} hint={outreachStats.followUps ? `${outreachStats.followUps} 条近期跟进` : '等待进展'}/></div><div className="target-toolbar"><div className="filter-chips">{['全部目标','中科院','高校'].map(x => <button key={x} className={targetFilter === x ? 'active' : ''} onClick={() => setTargetFilter(x)}>{x}</button>)}</div><div className="target-toolbar-actions"><span>{visibleTargets.length} 个目标单位</span><button onClick={() => openTarget()}><Plus size={14}/> 新增院校</button></div></div><div className="target-grid">{visibleTargets.map(target => <article className={`target-card ${target.group === '中科院/国科大' ? 'cas' : 'university'}`} key={target.id}><div className="target-title"><span><Building2 size={17}/></span><div><small>{target.group} · {target.city}</small><h3>{target.institution}</h3><p>{target.unit}</p></div><b>{target.match.fit}% 匹配</b></div><div className="tags">{target.directions.slice(0, 6).map(tag => <span key={tag}>{tag}</span>)}</div><p className="match-copy">{target.match.reasons.length ? `匹配：${target.match.reasons.join('、')}` : '建议结合导师论文进一步判断'}</p><p className="next-action"><Sparkles size={14}/><span><b>下一步：</b>{target.nextAction || '补充下一步行动，方便持续推进。'}</span></p><div className="target-actions"><button className="target-record" onClick={() => openOutreach(target)}><Mail size={15}/> 记录导师</button>{target.facultyUrl && <a href={target.facultyUrl} target="_blank" rel="noreferrer">导师入口 <ExternalLink size={13}/></a>}{target.admissionsUrl && <a href={target.admissionsUrl} target="_blank" rel="noreferrer">招生信息 <ExternalLink size={13}/></a>}<button className="target-icon-action" onClick={() => openTarget(target)} title="编辑院校" aria-label={`编辑 ${target.institution}`}><Pencil size={14}/></button><button className="target-icon-action danger" onClick={() => removeTarget(target.id)} title="删除院校" aria-label={`删除 ${target.institution}`}><Trash2 size={14}/></button></div></article>)}{!visibleTargets.length && <Empty icon={<GraduationCap/>} title="没有匹配的目标院校" text="清除搜索条件，或添加一所新的目标院校。"/>}</div><div className="outreach-list-head"><div><h3>套磁流水</h3><p>记录首次联系、下次跟进和回复状态</p></div><div className="select-wrap"><ListFilter size={15}/><select value={outreachFilter} onChange={e => setOutreachFilter(e.target.value)}><option>全部状态</option>{outreachStatusOptions.map(x => <option key={x}>{x}</option>)}</select><ChevronDown size={14}/></div></div><OutreachTable items={filteredOutreach} setItems={setOutreachRecords} onEdit={openOutreachEdit} onRemove={removeOutreach}/></section>
        </>}

        {activeNav === '投递管理' && <><PageTitle title="投递信息管理" description="按状态筛选，支持优先级、日期与公司排序。" icon={<BriefcaseBusiness/>}/><section className="panel"><div className="panel-head"><div><h2>全部投递</h2><p>{filteredApps.length} 条记录 · 第 {safeApplicationPage} / {applicationPageCount} 页</p></div><div className="panel-tools"><div className="select-wrap"><ListFilter size={15}/><select aria-label="按状态筛选" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option>全部状态</option>{statusOptions.map(x => <option key={x}>{x}</option>)}</select><ChevronDown size={14}/></div><div className="select-wrap"><CalendarClock size={15}/><select aria-label="投递排序方式" value={applicationSort} onChange={e => setApplicationSort(e.target.value as ApplicationSort)}><option value="date-desc">最新投递优先</option><option value="priority-desc">高优先级优先</option><option value="deadline-asc">截止日最近优先</option><option value="company-asc">公司名称排序</option></select><ChevronDown size={14}/></div></div></div><ApplicationsTable items={paginatedApps} setItems={setApplications} onEdit={openEdit} onRemove={removeApplication}/><Pagination currentPage={safeApplicationPage} totalPages={applicationPageCount} totalItems={filteredApps.length} pageSize={applicationPageSize} onPageChange={setApplicationPage} onPageSizeChange={setApplicationPageSize}/></section></>}

        {activeNav === '官网情报' && <><PageTitle title="官网招聘雷达" description="岗位匹配度由个人档案实时计算，招聘状态仍以企业官网为准。" icon={<Radar/>}/><section className="radar-section"><div className="panel-head"><div><h2>适合你的机会</h2><p>档案字段发生变化后自动重新排序</p></div><div className="filter-chips">{['为我推荐','央国企','AI / 算法','应届校招'].map(x => <button key={x} className={oppFilter === x ? 'active' : ''} onClick={() => setOppFilter(x)}>{x}</button>)}</div></div><div className="opportunity-grid">{visibleOpportunities.map(opp => <article className="opportunity" key={opp.id}><div className="opp-top"><span className="company-logo" style={{'--brand-color': opp.color} as React.CSSProperties}>{opp.short}</span><span className="fit"><Sparkles size={13}/> {opp.match.fit}% 匹配</span></div><h3>{opp.company}</h3><h4>{opp.role}</h4><p>{opp.category}</p><small>{opp.locations}</small><div className="match-reasons">{opp.match.reasons.map(x => <span key={x}>{x}</span>)}</div><div className="deadline"><CalendarDays size={16}/><div><small>网申状态</small><b>{opp.deadlineLabel}</b></div></div><div className="source-note"><Check size={13}/> 官方来源 · {opp.verifiedAt} 核验</div><div className="opp-actions"><button onClick={() => openAdd(opp)}><Plus size={16}/> 加入计划</button><a href={opp.officialUrl} target="_blank" rel="noreferrer">进入官网 <ExternalLink size={14}/></a></div></article>)}</div><div className="accuracy-note"><CircleHelp size={17}/><p><b>更新说明：</b>新增央企官方招聘入口。平台不推测未公开的截止日，请在投递前进入官网复核岗位与时间。</p></div></section></>}
    ...
    {targetOpen && <TargetModal form={targetForm} setForm={setTargetForm} editing={Boolean(editingTargetId)} onClose={() => setTargetOpen(false)} onSubmit={saveTarget}/>}
...
function TargetModal({ form, setForm, editing, onClose, onSubmit }: { form: PhdTargetForm, setForm: React.Dispatch<React.SetStateAction<PhdTargetForm>>, editing: boolean, onClose: () => void, onSubmit: (e: React.FormEvent) => void }) { return <div className="modal-backdrop" onMouseDown={onClose}><form className="modal" role="dialog" aria-modal="true" aria-label={editing ? '编辑目标院校' : '新增目标院校'} onSubmit={onSubmit} onMouseDown={e => e.stopPropagation()}><ModalHead icon={<Building2/>} title={editing ? '编辑目标院校' : '新增目标院校'} text="院校信息会自动保存到当前账号" onClose={onClose}/><div className="form-grid"><label>目标类型<select value={form.group} onChange={e => setForm({ ...form, group: e.target.value as PhdTarget['group'] })}><option>高校</option><option>中科院/国科大</option></select></label><label>院校 / 研究所名称 *<input required value={form.institution} onChange={e => setForm({ ...form, institution: e.target.value })}/></label><label>城市<input value={form.city} onChange={e => setForm({ ...form, city: e.target.value })}/></label><label>院系 / 单位<input value={form.unit} onChange={e => setForm({ ...form, unit: e.target.value })}/></label><label className="wide">研究方向<input value={form.directions} onChange={e => setForm({ ...form, directions: e.target.value })} placeholder="多个方向用顿号或逗号分隔"/></label><label>申请优先级<input value={form.priority} onChange={e => setForm({ ...form, priority: e.target.value })} placeholder="例如：冲刺 / 重点 / 稳健"/></label><label>建议导师<input value={form.suggestedMentor || ''} onChange={e => setForm({ ...form, suggestedMentor: e.target.value })}/></label><label>招生信息网址<input type="url" value={form.admissionsUrl} onChange={e => setForm({ ...form, admissionsUrl: e.target.value })}/></label><label>导师入口网址<input type="url" value={form.facultyUrl} onChange={e => setForm({ ...form, facultyUrl: e.target.value })}/></label><label className="wide">下一步行动<textarea value={form.nextAction} onChange={e => setForm({ ...form, nextAction: e.target.value })}/></label><label className="wide">备注<textarea value={form.note} onChange={e => setForm({ ...form, note: e.target.value })}/></label></div><ModalActions editing={editing} onClose={onClose}/></form></div> }
function ModalHead({ icon, title, text, onClose }: { icon: React.ReactNode, title: string, text: string, onClose: () => void }) { return <div className="modal-head"><div><span>{icon}</span><div><h2>{title}</h2><p>{text}</p></div></div><button type="button" onClick={onClose} aria-label="关闭弹窗"><X/></button></div> }
